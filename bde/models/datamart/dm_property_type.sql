 ...
 with dm_property_type as (
  select
    ...
    count(distinct host_id) as distinct_host,
 	...
 	round((lead(sum(case when l.has_availability = true then 1 else 0 end)) over (partition by p.property_type  order by to_char(l.scraped_date, 'MM/YYYY')) - sum(case when l.has_availability = true then 1 else 0 end))::numeric / nullif(sum(case when l.has_availability = true then 1 else 0 end),0)::numeric *100,2) as active_change,
 	round((lead(sum(case when l.has_availability = false then 1 else 0 end)) over (partition by p.property_type order by to_char(l.scraped_date, 'MM/YYYY')) - sum(case when l.has_availability = false then 1 else 0 end))::numeric / nullif(sum(case when l.has_availability = true then 1 else 0 end),0)::numeric *100,2) as inactive_change,
