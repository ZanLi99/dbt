...
def import_date_func_house(**kwargs):
    ...
    ps_pg_hook = PostgresHook(postgres_conn_id="postgres")
    conn_ps = ps_pg_hook.get_conn()
    df_01 = pd.read_csv(LISTINGS+'01_2021.csv')
    df_02 = pd.read_csv(LISTINGS+'02_2021.csv')
    df_03 = pd.read_csv(LISTINGS+'03_2021.csv')
    df_04 = pd.read_csv(LISTINGS+'04_2021.csv')
    df_05 = pd.read_csv(LISTINGS+'05_2020.csv')
    df_06 = pd.read_csv(LISTINGS+'06_2020.csv')
    df_07 = pd.read_csv(LISTINGS+'07_2020.csv')
    df_08 = pd.read_csv(LISTINGS+'08_2020.csv')
    df_09 = pd.read_csv(LISTINGS+'09_2020.csv')
    df_10 = pd.read_csv(LISTINGS+'10_2020.csv')
    df_11 = pd.read_csv(LISTINGS+'11_2020.csv')
    df_12 = pd.read_csv(LISTINGS+'12_2020.csv')
    dfs = [df_01, df_02, df_03, df_04, df_05, df_06, df_07, df_08, df_09, df_10, df_11, df_12]
    df_code = pd.concat(dfs, ignore_index=True,axis=0)
    df_code['HOST_NAME'].fillna('',inplace=True)
    df_code['HOST_SINCE'].fillna('',inplace=True)
    df_code['HOST_IS_SUPERHOST'].fillna(False,inplace=True)
    df_code['HOST_NEIGHBOURHOOD'].fillna('',inplace=True)
    df_code['REVIEW_SCORES_RATING'].fillna(0,inplace=True)
    df_code['REVIEW_SCORES_ACCURACY'].fillna(0,inplace=True)
    df_code['REVIEW_SCORES_CLEANLINESS'].fillna(0,inplace=True)
    df_code['REVIEW_SCORES_CHECKIN'].fillna(0,inplace=True)
    df_code['REVIEW_SCORES_COMMUNICATION'].fillna(0,inplace=True)
    df_code['REVIEW_SCORES_VALUE'].fillna(0,inplace=True)
    return df_code
# ------------------------------------------------------
